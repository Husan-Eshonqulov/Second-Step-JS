...
const createHolidaysHTML = (holiday) => {
    return `<p><span class="left">"<b>${holiday.name}</b>"</span> <span class="right">${holiday.date}</span></p>`;
};
...
function executeSearch(executeCountry, executeYear) {
    const result = getHolidays(urlToFetchFun(executeCountry, executeYear)).then(holidays => renderHolidays(holidays));
    console.log(result);
    return result;
}